...
{
  public:
    encoder(): Node("lgv_encoders_hw")
    {
        RCLCPP_INFO(this->get_logger(), "init class:");
    ...
    public:
       ...
       ,std::shared_ptr<ros2_control_bridge::srv::Encoderservice::Response>  response)
      {
            if(request->state)
            {
                response->to_encoder_left = (double)temp1;
                response->to_encoder_right = (double)temp2;
...
int main(int argc, char * argv[])
{
  ...
  //rclcpp::RCLCPP_INFO(encoder().get_logger(), "encoder_left: '%d'", msg.data);
  rclcpp::shutdown();
  return 0;
}
